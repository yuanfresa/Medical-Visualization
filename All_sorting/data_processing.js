...
{
	var markersorting_array = [];
	for(var i = 0; i < cluster_names.length; i++)
		markersorting_array.push(ranking_score_pair_marker[index][i][1]);

	return markersorting_array;
}

function ClusterSorting(index)
{
	var clustersorting_array = [];
	for(var j = 0; j < marker_names.length; j++)
		clustersorting_array.push(ranking_score_pair[index][j][1]);
	return clustersorting_array;
}
...
for (var i = 0; i < cluster_names.length; i++)
{
	...
	{
		variance_nor[i][j] = [];
		variance_nor[i][j] = (-variance_matrix[i][j][0] + variance_max)/(variance_max-variance_min);	
	}
}
// record the index, stay the same	
var hcrow = [],
	hccol = [];
for (var i = 0; i < cluster_names.length; i++) {
	hcrow[i] = i;
...
function DataForHeatmap(x,y ){ //(newhcrow, newhccol)
	var data_heatmap = [];
	for (var i = 0; i < hcrow.length; i++)
	{
	  for (var j = 0; j < hccol.length; j++)
	  {
	    data_heatmap.push({
	      "dim_col": j,
	      "dim_row": i,
	      "mean": mean_matrix[x[i]][y[j]][0],
	      "variance" : variance_matrix[x[i]][y[j]][0],
	      "variance_nor" :  variance_nor[x[i]][y[j]]
	    });
	  }
	}
	return data_heatmap;
...
function DataForRowLabel(x) { //newhcrow
	var data_rowlabel = [];
	for (var i = 0; i < x.length; i++)
	{
	    data_rowlabel.push({
	      "name": cluster_names[x[i]],
	      "sort": i,
	      "index": x[i] // call other function
	    });
	}
	return data_rowlabel;	
}
function DataForColLabel(y) {
	var data_collabel = [];
	for (var j = 0; j < y.length; j++)
	{
	    data_collabel.push({
	      "name": marker_names[y[j]],
	      "sort": j,
	      "index" : y[j] // call other function
	    });
	}
	return data_collabel;	
}
